...
// xについて定積分を求める関数
public class Ix extends Function3 {
	...
	private double simpson (Evaluable f, double a, double b) {
		SpecialVariable.variables[0] = new Real(a);
		double fa = f.evaluate().toReal().getX();
		SpecialVariable.variables[0] = new Real((a + b) / 2);
		double fab = f.evaluate().toReal().getX();
		SpecialVariable.variables[0] = new Real(b);
